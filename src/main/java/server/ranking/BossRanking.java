...
public final class BossRanking {
    ...
    private record BossScore(String name, int points) {}

    private static final Map<Integer, BossScore> BOSS_POINTS = Map.ofEntries(
            Map.entry(MobId.ZAKUM_3, new BossScore("Zakum", 10)),
            Map.entry(MobId.PAPULATUS, new BossScore("Papulatus", 7)),
            Map.entry(MobId.PIANUS_R, new BossScore("Pianus", 5)),
            Map.entry(MobId.PIANUS_L, new BossScore("Pianus", 5)),
            Map.entry(MobId.HORNTAIL, new BossScore("Horntail", 25)),
            Map.entry(MobId.PINK_BEAN, new BossScore("Pink Bean", 30)),
            Map.entry(MobId.FURIOUS_SCARLION, new BossScore("Scarlion", 15)),
            Map.entry(MobId.FURIOUS_TARGA, new BossScore("Targa", 15))
    );
